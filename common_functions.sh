...
function set_arch_os() {
	...
	armv7l)
		current_arch="armv7l"
		oses="ubuntu debian debian-slim centos"
		os_family="linux"
		;;
	aarch64)
		current_arch="aarch64"
		oses="ubuntu debian debian-slim ubi ubi-minimal centos"
		os_family="linux"
		;;
	ppc64el|ppc64le)
	...
	s390x)
		current_arch="s390x"
		oses="ubuntu debian debian-slim ubi ubi-minimal clefos"
		os_family="linux"
		;;
	amd64|x86_64)
		case $(uname) in
			...
			*)
			current_arch="x86_64"
			oses="ubuntu alpine debian debian-slim ubi ubi-minimal centos"
			os_family="linux"
			;;
		esac
		;;
